...
#-----------------------------------------------------------Метод append и extend-----------------------------------------------------------------------------
# Метод append добовляет в конец списка новый эелемент
a = ['Hello', "World"]
a.append('Этот элемент был добавлен!')
print(a)
# Метод extend даёт возможность добовлять элементы из одного списка в другой
# Если мы добавим элемены из одного списка в другой методом append то мы получим список списков|Пример:
b = ['goodbye']
rl = [] # Пустой список в которы мы будем обединять спски a и b c помощю append
rl2 = [] # Пустой список в которы мы будем обединять спски a и b c помощю extend
# --------------
rl.append(a)
rl.append(b)
print(rl)
